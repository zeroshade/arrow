...
func ToScalar(opt interface{}, mem memory.Allocator) (Scalar, error) {
	v := reflect.Indirect(reflect.ValueOf(opt))
	switch v.Kind() {
	case reflect.Struct:
	...
	return MakeScalar(opt), nil
}

func createListScalar(sliceval reflect.Value, mem memory.Allocator) (Scalar, error) {
	if sliceval.Kind() != reflect.Slice {
		return nil, xerrors.Errorf("createListScalar only works on slices, not %s", sliceval.Kind())
	}
...
func MakeScalar(val interface{}) Scalar {
	...
	panic(xerrors.Errorf("makescalar not implemented for type %#T", val))
}
