...
func concatBitmaps(bitmaps []bitmap, mem memory.Allocator) (*memory.Buffer, error) {
	...
	for _, bm := range bitmaps {
		if bm.data == nil {
			bitutil.SetBitsTo(out.Bytes(), offset, int64(bm.rng.len), true)
		} else {
			bitutil.CopyBitmap(bm.data, int64(bm.rng.offset), int64(bm.rng.len), dst, offset)
		}
		offset += int64(bm.rng.len)
	}
	return out, nil
}
